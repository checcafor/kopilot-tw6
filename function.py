...
def ride_already_exits(_id_Viaggio, _idUtente):
    ...
    query = """
                ...
                WHERE
                _idViaggio = ? AND _idUtente = ? AND (Stato = 'Accepted' OR Stato = 'Pending...')
            """
...
def end_trip(trip_id):
    ...
    if trip_already_ended(trip_id):
        ...
        cursor.execute("UPDATE Viaggi SET Stato = 'Finished' WHERE idViaggio = ?", ( trip_id, ))    # removal of an available seat for that trip
        connection.commit() # the changes generated by the previous queries are committed
        connection.close()  # the connection with database is closed
        return
# ----------------------------------------------------------------
...
def get_notifications():
    ...
    query = """ SELECT * 
                ...
                WHERE r.Stato = 'Pending...' AND v._idUtente = ?
            """
...
def accept_request_byID(req_id):
    ...
    # query to update the status of a given request to 'Accepted'
    cursor.execute("UPDATE RichiestePassaggio SET Stato = 'Accepted' WHERE idRichiestaPassaggio = ? ", (req_id,))  # the query is executed to update the status of the request
    connection.commit()                 # the changes generated by the previous queries are committed
    connection.close()                  # the connection with database is closed
    return
# ----------------------------------------------------------------
...
def reject_request_byID(req_id):
    ...
    # query to update the status of a given request to 'Rejected'
    cursor.execute("UPDATE RichiestePassaggio SET Stato = 'Rejected' WHERE idRichiestaPassaggio = ? ", (req_id,))      # the query is executed to update the status of the request

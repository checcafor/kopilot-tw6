...
def give():
    ...
    if request.method == 'POST' and give_form.validate_on_submit():
        ...
        if trip_already_exists(_idUtente,start_pos,start_lat,start_lng, posti,timestamp):
            ...
            return render_template("give.html", message = None, form = give_form, error='Il viaggio che stai cercando di inserire, è già stato registrato', notificationsNumber = notificationsNumber)
        else:
            connection = connect("database.db")     # a connection is established to the database called "database.db"
            cursor = connection.cursor()            # a cursor object is created to execute queries
            ...
            return render_template('give.html', message = "Viaggio registrato con successo", form = give_form, error = None, notificationsNumber = notificationsNumber)
    else:
...
def take():
    ...
    if request.method == 'POST' and take_form.validate_on_submit():
        ...
        if not trip_exists(_idUtente,user_lat, user_lng):
            ...
            return render_template("take.html", error='Nessun viaggio con i criteri richiesti nelle vicinanze', message = None, selected_trip = None, form = take_form, notificationsNumber = notificationsNumber)
        else:

...
def login():
    ...
    if request.method == 'POST' and login_form.validate_on_submit():
        ...
        if user_info:
            ...
            return render_template("login.html", error = "wrong credentials, please try again", message = None, form = login_form)
...
def signup():
    ...
    if request.method == 'POST' and singup_form.validate_on_submit():  
        ...
        if username_already_exists(email):
            ...
            return render_template("signup.html", message='the email you are trying to enter has already been used for another user profile', form = singup_form)
        # if a user with that phone number already exists
        elif phone_already_exists(phone):
            # the singup page is rendered with an error message
            return render_template("signup.html", message='the phone you are trying to enter has already been used for another user profile', form = singup_form)
        else:
            ...
            return render_template('login.html', message = "Registration successful, proceed to login", error = None, form = login_form)
    else:
...
def home():
    ...
    return render_template("home.html", notificationsNumber = notificationsNumber) 
# ----------------------------------------------------------------
...
def history():
    ...
    return render_template("history.html", trips=user_trips, rides = user_rides, notificationsNumber = notificationsNumber) 
# ----------------------------------------------------------------
...
def request_ride_route():
    ...
    if ride_already_exits(ride_id, _idUtente):
        ...
        return render_template("take.html", error='You have already booked this ride', message = None, selected_trip = None, form = take_form)
    else:
        cursor.execute("INSERT INTO RichiestePassaggio (_idViaggio, _idUtente, indirizzoPartenzaRider, indirizzoArrivo, lat_Arrivo, lon_Arrivo, Stato) VALUES (?,?,?,?,?,?,?);", ( ride_id, _idUtente, startAddress, destAddress, latAddress, lonAddress, "Pending...")) # query to add a ride request waiting to be accepted to the db
        ...
        return render_template('take.html', message = "Ride requested successfully", error = None, form = take_form)
# ----------------------------------------------------------------
...
def end_trip_route(trip_id):
    ...
    return render_template('home.html', message = "Trip completed successfully", error = None)
# ----------------------------------------------------------------
...
def give():
    ...
    if request.method == 'POST' and give_form.validate_on_submit():
        ...
        if trip_already_exists(_idUtente,start_pos,start_lat,start_lng, posti,timestamp):
            ...
            return render_template("give.html", message = None, form = give_form, error='The trip you are trying to enter has already been registered', notificationsNumber = notificationsNumber)
        else:
            connection = connect("database.db")     # a connection is established to the database called "database.db"
            cursor = connection.cursor()            # a cursor object is created to execute queries
            ...
            cursor.execute("INSERT INTO Viaggi (_idUtente,indirizzoPartenza,lat_Partenza,lon_Partenza,time_stamp,postiDisponibili, Stato) VALUES (?, ?, ?, ?, ?, ?, ?)", 
                           (_idUtente,start_pos,start_lat,start_lng,timestamp,posti, "In progress..."))
            connection.commit() # the changes generated by the previous query are committed
            connection.close()  # the database connection is closed
            # the same page is returned with a message confirming registration
            return render_template('give.html', message = "Trip registered successfully", form = give_form, error = None, notificationsNumber = notificationsNumber)
    else:
        # if the request method is "GET" it means that the user is simply accessing the page
        return render_template("give.html", message = None, form = give_form, error = None, notificationsNumber = notificationsNumber)
# ----------------------------------------------------------------
...
def take():
    ...
    if request.method == 'POST' and take_form.validate_on_submit():
        ...
        if not trip_exists(_idUtente,user_lat, user_lng):
            ...
            return render_template("take.html", error='No trips with the required criteria nearby', message = None, selected_trip = None, form = take_form, notificationsNumber = notificationsNumber)
        else:
            ...
            return render_template('take.html', available_trips = available_trips, error = None, message = None, form = take_form, request_info = request_info, notificationsNumber = notificationsNumber)
    else:
        # if the request method is "GET", then the user is simply accessing the page
        return render_template("take.html", error = None, form = take_form, message = None, notificationsNumber = notificationsNumber)
# ----------------------------------------------------------------
        ...
        if not user:
            ...
            if not user:
                ...
                if user:    # if the user has not changed any parameters
                    ...
                    return render_template("profile.html", user_info = user_info, message = None, info_error = "Edit some fields to make the change effective", form = form, notificationsNumber = notificationsNumber, error = None)
                    ...
                    return render_template("profile.html", user_info = user_info, message = "Credentials updated successfully", error = None, form = form, notificationsNumber = notificationsNumber, info_error = None)   
            else:
                # if there is at least one user with this phone number
                connection.close()  # the database connection is closed
                # the same page is returned showing an error message
                return render_template("profile.html", user_info = user_info, message = None, error = "Please note, this phone number has already been used", form = form, notificationsNumber = notificationsNumber, info_error = None)
            
        else:
            # if there is at least one user with this email
            connection.close()  # the database connection is closed
            # the same page is returned showing an error message
            return render_template("profile.html", user_info = user_info, message = None, error = "Warning, this email has already been used", form = form, notificationsNumber = notificationsNumber, info_error = None)
...
def requests():
    ...
    return render_template("requests.html", notifications = notifications, notificationsNumber = notificationsNumber) 
# ----------------------------------------------------------------
